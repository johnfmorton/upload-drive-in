<section>
    ...
    <form id="send-verification" method="post" action="{{ route('employee.verification.send', ['username' => auth()->user()->username]) }}">
        @csrf
    </form>

    <form method="post" action="{{ route('employee.profile.update', ['username' => auth()->user()->username]) }}" class="mt-6 space-y-6">
        @csrf
        @method('patch')
