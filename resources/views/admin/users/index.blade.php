        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                ...
                <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4" x-data="{
                    ...
                    <div>
                        <label for="name"
                            class="block text-sm font-medium text-gray-700">{{ __('messages.label_name') }}</label>
                    ...
                    <div>
                        <label for="email"
                            class="block text-sm font-medium text-gray-700">{{ __('messages.label_email') }}</label>
                    ...
                    <div class="flex flex-col sm:flex-row gap-3">
                        <button type="button" @click="submitForm('create')" :disabled="submitting"
                        ...
                        <button type="button" @click="submitForm('create_and_invite')" :disabled="submitting"
            ...
            <!-- Users Table Section -->
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div x-data="adminUsersData()" x-init="initData()" class="max-w-full">
                    <h2 class="text-lg font-medium text-gray-900 mb-4">
                    ...
                    <div x-show="showDeleteModal" x-transition:enter="ease-out duration-300"
                        ...
                        style="display: none;" @keydown.escape.window="showDeleteModal = false">
                        ...
                        <div class="relative bg-white rounded-lg shadow-xl p-6 m-4 max-w-md w-full" @click.stop>
                            ...
                            <div class="mt-2">
                                ...
                                <label class="flex items-center space-x-2 mb-6">
                                    <input type="checkbox" x-model="deleteFilesCheckbox"
                                        class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                                    <span
                                        class="text-sm text-gray-700">{{ __('messages.delete_user_modal_checkbox') }}</span>
                                </label>
                            </div>
                            <div
                                ...
                                <button @click="showDeleteModal = false; userToDeleteId = null;" type="button"
                                    class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--brand-color)] sm:mt-0 sm:w-auto sm:text-sm">
                    ...
                    <div class="mb-4 p-4 border rounded bg-gray-50 hidden lg:block">
                        <h3 class="text-md font-medium text-gray-700 mb-2">
                            {{ __('messages.toggle_user_columns_title') }}</h3>
                        <div class="grid grid-cols-2 sm:grid-cols-5 gap-2 text-sm">
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" x-model="columns.name"
                                    class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                                <span>{{ __('messages.label_name') }}</span>
                            </label>
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" x-model="columns.email"
                                    class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                                <span>{{ __('messages.label_email') }}</span>
                            </label>
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" x-model="columns.createdAt"
                                    class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                                <span>{{ __('messages.column_created_at') }}</span>
                            </label>
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" x-model="columns.loginUrl"
                                    class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                                <span>{{ __('messages.column_login_url') }}</span>
                            </label>
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" x-model="columns.actions"
                                    class="rounded border-gray-300 text-[var(--brand-color)] shadow-sm focus:ring-[var(--brand-color)]">
                    ...
                    <div class="mb-4 space-y-4">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div class="flex items-center space-x-4">
                                <label
                                    class="text-sm font-medium text-gray-700">{{ __('messages.filter_by_primary_contact') }}:</label>
                                <div class="flex items-center space-x-2">
                                    ...
                                    <a href="{{ route('admin.users.index', array_filter(['filter' => 'primary_contact', 'search' => request('search')])) }}"
                        ...
                        <!-- Search Input -->
                        <div>
                            <form method="GET" action="{{ route('admin.users.index') }}" x-ref="searchForm">
                                <!-- Preserve existing filters but explicitly exclude page parameter -->
                                @if(request('filter'))
                            ...
                            <div class="bg-white p-4 rounded-lg shadow border border-gray-200">
                                ...
                                <div class="border-b border-gray-200 pb-3 mb-3">
                                    <div class="flex justify-between items-start gap-2">
                                        <div class="flex items-center space-x-2">
                                            ...
                                            @if($client->is_primary_contact_for_current_user)
                                                <span
                                        ...
                                        <div class="flex shrink-0 space-x-2">
                                            ...
                                            <a href="{{ route('admin.users.show', $client->id) }}"
                                                class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-[var(--brand-color)] bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--brand-color)]">
                                                Manage
                                            </a>
                                            {{-- Copy URL Button --}}
                                            <button @click="copyLoginUrl({{ json_encode($client) }})"
                                                class="inline-flex items-center px-2.5 py-1.5 border border-gray-300 shadow-sm text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--brand-color)]">
                                                <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor"
                                                ...
                                                <span
                                                    x-show="copiedUrlId !== {{ $client->id }}">{{ __('messages.button_copy_login_url') }}</span>
                                                <span x-show="copiedUrlId === {{ $client->id }}"
                                                    class="text-green-600">{{ __('messages.copied_confirmation') }}</span>
                                            </button>
                                            {{-- Delete Button --}}
                                            <button @click="deleteUser({{ $client->id }})"
                                ...
                                <div class="space-y-2 text-sm">
                                    <div class="flex justify-between">
                                        ...
                                        <span class="font-medium text-gray-900 truncate">{{ $client->email }}</span>
                                    </div>
                                    <div class="flex justify-between">
                                        <span
                                            class="text-gray-500">{{ __('messages.mobile_label_created_at') }}</span>
                                    ...
                                    <tr>
                                        <template x-if="columns.name">
                                            <th scope="col"
                                                ...
                                                @click="sortBy('name')">
                                                {{ __('messages.label_name') }} <span x-show="sortColumn === 'name'"
                                        ...
                                        <template x-if="columns.email">
                                            <th scope="col"
                                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                                                @click="sortBy('email')">
                                                {{ __('messages.label_email') }} <span x-show="sortColumn === 'email'"
                                                    x-text="sortDirection === 'asc' ? '▲' : '▼'"></span>
                                            </th>
                                        </template>
                                        <template x-if="columns.createdAt">
                                            <th scope="col"
                                                ...
                                                {{ __('messages.column_created_at') }} <span
                                                    x-show="sortColumn === 'created_at'"
                                                    x-text="sortDirection === 'asc' ? '▲' : '▼'"></span>
                                            </th>
                                        </template>
                                        <template x-if="columns.loginUrl">
                                            <th scope="col"
                                                ...
                                                {{ __('messages.column_login_url') }}</th>
                                        </template>
                                        <template x-if="columns.actions">
                                            <th scope="col"
                                                class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                                {{ __('messages.column_actions') }}</th>
                                        </template>
                                    </tr>
                                </thead>
                                        ...
                                        <tr>
                                            ...
                                            <template x-if="columns.actions">
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                                    <a href="{{ route('admin.users.show', $client->id) }}"
                                                        class="text-[var(--brand-color)] hover:text-[var(--brand-color)]/80">Manage</a>
                                                    <button @click="deleteUser({{ $client->id }})"
                                                        class="text-red-600 hover:text-red-900">{{ __('messages.delete_button') }}</button>
                                                </td>
                                            </template>
                                        </tr>
                                    @empty
    ...
    <script>
        function adminUsersData() {
            return {
                ...
                copyLoginUrl(client) {
                    const loginUrl = client.login_url || client.loginUrl;
                ...
                initData() {
                    // No longer needed for client-side filtering
                    this.copiedUrlId = null;
                }
            }
        }
