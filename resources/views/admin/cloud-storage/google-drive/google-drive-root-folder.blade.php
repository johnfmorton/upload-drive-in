...
<div x-data="googleDriveFolderPicker()" x-init="init()">
    <form id="google-drive-folder-form" action="{{ route('admin.cloud-storage.google-drive.folder.update') }}" method="POST" class="space-y-4">
        @csrf
        @method('PUT')
            ...
            <div class="flex">
                ...
                <div class="ml-3">
                    ...
                    <div class="mt-2 text-sm text-blue-700">
                        <p>Your root folder setting is stored in your user profile. If no folder is selected, uploads
                            will go to your Google Drive root directory by default.</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center space-x-2 border-gray-300 rounded-md border p-4">
            <input type="hidden" id="google_drive_root_folder_id" name="google_drive_root_folder_id"
                x-model="currentFolderId" />
            <span class="text-gray-700 relative top-[1px]">📁</span>
            <span x-text="currentFolderName || 'Google Drive Root (default)'" class="inline-block w-full text-gray-700">
            </span>
            <x-secondary-button type="button" @click="openModal"
            ...
            <div @click.away="closeModal" class="bg-white rounded-lg shadow-lg w-3/4 max-w-2xl p-6">
                ...
                <div class="flex items-center justify-between mb-4">
                    <button type="button" @click="goUp"
                        class="px-2 py-1 bg-gray-200 rounded">{{ __('messages.up') }}</button>
                    <span class="text-lg font-medium"
                        x-text="folderStack.length ? folderStack[folderStack.length - 1].name : rootFolderName"></span>
                    <button type="button" @click="closeModal"
                        class="px-2 py-1 bg-gray-200 rounded">{{ __('messages.cancel') }}</button>
                </div>
                ...
                <div class="flex items-center space-x-2 mb-4">
                    <input x-model="newFolderName" type="text" placeholder="{{ __('messages.create_new_folder') }}"
                        class="mt-0 block w-full border-gray-300 rounded-md" />
                    <x-button type="button" @click="createFolder"
                        class="bg-green-600 hover:bg-green-700 text-white whitespace-nowrap px-2 py-1 rounded">
                        {{ __('messages.create_folder') }}
                    </x-button>
                </div>
                <div class="flex justify-between">
                    <x-secondary-button type="button" @click="useGoogleDriveRoot"
                    ...
                    <div class="flex space-x-2">
                        ...
                        <x-secondary-button type="button"
                            @click="closeModal">{{ __('messages.cancel') }}</x-secondary-button>
                    </div>
                </div>
            </div>
        </div>
