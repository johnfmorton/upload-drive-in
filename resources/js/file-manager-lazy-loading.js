...
class FileManagerLazyLoader {
    constructor(options = {}) {
        this.container = options.container || document.querySelector('[data-lazy-container]');
        this.loadMoreButton = options.loadMoreButton || document.querySelector('[data-load-more]');
        ...
        try {
            ...
            if (response.success) {
                ...
                this.cache.set(nextPage, {
                    ...
                    hasMore: response.files.has_more_pages || response.files.next_page_url !== null
                });
                
                this.appendFiles(response.files.data);
                this.currentPage = nextPage;
                this.hasMoreItems = response.files.has_more_pages || response.files.next_page_url !== null;
            ...
            if (loadTime > 1000) {
                console.warn(`Slow page load: ${loadTime}ms for page ${nextPage}`);
            }
        }
    }
    ...
    appendFiles(files) {
        ...
        // Trigger any Alpine.js re-initialization if needed
        if (window.Alpine) {
    ...
    populateFileData(element, file) {
        ...
        if (statusEl) {
            ...
            statusEl.className = file.is_pending ? 'status-pending' : 'status-completed';
        }
    }
    ...
    setLoading(loading) {
        ...
        if (this.loadMoreButton) {
            ...
            this.loadMoreButton.textContent = loading ? 'Loading...' : 'Load More';
        }
    }
    ...
    getTotalItemCount() {
        ...
        return this.container ? this.container.querySelectorAll('[data-file-item]').length : 0;
    }
    ...
    optimizeMemory() {
        ...
        if (this.cache.size > maxCacheSize) {
            const oldestKeys = Array.from(this.cache.keys()).slice(0, this.cache.size - maxCacheSize);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (lazyContainer) {
        window.fileManagerLazyLoader = new FileManagerLazyLoader();
    }
});
